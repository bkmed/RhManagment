...
import { Users, FileText, Calendar, DollarSign, AlertCircle, Settings, UserPlus, Shield } from "lucide-react-native"
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [fetchedEmployees, fetchedLeaveRequests, fetchedPayslips, fetchedIllnessRecords] = await Promise.all([
          getAllEmployees(),
  ...
  const renderWelcomeCard = () => {
    return (
      <Card style={styles.welcomeCard}>
        <Text style={styles.welcomeTitle}>Welcome, {user?.displayName || "Admin"}</Text>
        <Text style={styles.welcomeSubtitle}>System Administrator Dashboard</Text>
      </Card>
    )
  }
  ...
  const renderStatCards = () => {
    const stats = [
      ...
      },
      {
        ...
        icon: <DollarSign size={24} color="#38a169" />,
        color: "#f0fff4",
  ...
  const renderQuickActions = () => {
    const actions = [
      ...
      },
      {
        ...
        icon: <Shield size={24} color="#805ad5" />,
    ...
    return (
      <Card style={styles.card}>
        ...
        {illnessRecords.length === 0 ? (
          ...
          illnessRecords.slice(0, 3).map((illness) => {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.row}>
        ...
        <View style={styles.column}>{renderActiveIllnesses()}</View>
      </View>
    </ScrollView>
  )
}
