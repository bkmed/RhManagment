...
export const remoteDb = {
    ...
    addOrUpdate: async (remote: Omit<RemoteWork, 'id' | 'createdAt' | 'updatedAt'>): Promise<void> => {
        const data = getAllRemote();
        const now = new Date().toISOString();
        const index = data.findIndex(r => r.employeeId === remote.employeeId && r.date === remote.date);
