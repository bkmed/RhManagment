...
export const ChatBot = () => {
  ...
  const generateResponse = (
    ...
    const checkIntent = (keywords: string[]) =>
    ...
    if (user?.role === 'admin') {
      if (checkIntent(INTENTS.approvals)) {
        ...
        return {
          ...
          action: {
            label: t('chatBot.pendingApprovalsAction'),
            screen: 'LeavesTab',
          },
        };
      }
      ...
      if (checkIntent(INTENTS.approvals) || input.includes('status')) {
        const myPendingLeaves = pendingLeaves.filter(
          l => String(l.employeeId) === String(user?.id),
        ).length;
        const myPendingClaims = pendingClaims.filter(
          c => String(c.employeeId) === String(user?.id),
        ).length;
    ...
    if (checkIntent(INTENTS.team)) {
      ...
      if (myTeam) {
        return {
          text: t('chatBot.team', {
            team: myTeam.name,
            dept: myTeam.department,
          }),
          action: {
            label: t('chatBot.teamAction'),
            screen: 'Profile',
            subScreen: 'MyTeam',
          },
        };
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
        ...
        >
          <View style={styles.headerInfo}>
            <Text style={styles.headerTitle}>
              {t('common.assistant') || 'Assistant HR Pro'}
            </Text>
        ...
        {isTyping && (
          <Text
            style={[styles.typingIndicator, { color: theme.colors.subText }]}
          >
            Assistant is typing...
          </Text>
        )}
