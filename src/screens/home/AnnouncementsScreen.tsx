...
export const AnnouncementsScreen = () => {
  ...
  const [newCategory, setNewCategory] = useState<'news' | 'event' | 'alert'>(
    'news',
  );
  ...
  const renderItem = ({ item }: { item: Announcement }) => (
    ...
    >
      ...
      <Text
        style={[styles.content, { color: theme.colors.text, opacity: 0.8 }]}
      >
        {item.content}
      </Text>
  ...
  return (
    ...
    >
      <View style={styles.header}>
        {isAdmin && (
          <TouchableOpacity
            onPress={() => dispatch(setSelectedCompanyId(null))}
      ...
      <FlatList
        data={[...filteredAnnouncements].sort(
          (a, b) =>
