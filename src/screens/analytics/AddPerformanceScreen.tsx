...
import { PerformanceReview, Company, Team, Employee } from '../../database/schema';
...
export const AddPerformanceScreen = ({ route, navigation }: any) => {
    ...
    const validate = () => {
        ...
        if (!selectedEmployeeId) newErrors.employee = t('performance.errorEmployeeRequired');
        if (!period.trim()) newErrors.period = t('performance.errorPeriodRequired');
        if (!comments.trim()) newErrors.comments = t('performance.errorCommentsRequired');
    ...
    const handleSave = async () => {
        ...
        try {
            const reviewData: PerformanceReview = {
                ...
                score: Number(score),
                comments,
                date: isEditing ? editingReview!.date : new Date().toISOString().split('T')[0],
                createdAt: isEditing ? editingReview!.createdAt : new Date().toISOString(),
            };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                {user?.role === 'admin' && (
                    <>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.selectorScroll}>
                            ...
                            >
                                <Text style={[styles.chipText, tempCompanyId === 'none' && styles.activeChipText]}>
                                    {t('common.none')}
                                </Text>
                            </TouchableOpacity>
                            {companies.map((c: Company) => (
                                ...
                                >
                                    <Text style={[styles.chipText, tempCompanyId === c.id && styles.activeChipText]}>
                                        {c.name}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                ...
                {/* Team Selector */}
                {user?.role !== 'manager' && ((tempCompanyId && tempCompanyId !== 'none') || user?.role === 'rh') && (
                    <>
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.selectorScroll}>
                            ...
                            >
                                <Text style={[styles.chipText, tempTeamId === 'none' && styles.activeChipText]}>
                                    {t('common.none')}
                                </Text>
                            </TouchableOpacity>
                            {filteredTeams.map((t_item: Team) => (
                                <TouchableOpacity
                                    ...
                                    style={[styles.chip, tempTeamId === t_item.id && styles.activeChip]}
                                >
                                    <Text style={[styles.chipText, tempTeamId === t_item.id && styles.activeChipText]}>
                                        {t_item.name}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.selectorScroll}>
                    {filteredEmployees.map((emp: Employee) => (
                        ...
                        >
                            <Text style={[styles.chipText, selectedEmployeeId === emp.id && styles.activeChipText]}>
                                {emp.name}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                <View style={styles.scoreContainer}>
                    {['1', '2', '3', '4', '5'].map(val => (
                        ...
                        >
                            <Text style={[styles.scoreOptionText, score === val && styles.activeScoreText]}>
                                {val}
                            </Text>
                        </TouchableOpacity>
                    ))}
