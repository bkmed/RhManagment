...
import { selectAllDevices, updateDeviceStatus } from '../../store/slices/devicesSlice';
...
export const MyDevicesScreen = () => {
    ...
    const handleReportStatus = (id: number, currentCondition: 'working' | 'faulty') => {
        ...
        showModal({
            ...
            message: t('devices.confirmReport', { condition: t(`devices.${newCondition}`) }),
            buttons: [
                { text: t('common.cancel'), style: 'cancel' },
                {
                    text: t('common.yes'),
                    onPress: () => dispatch(updateDeviceStatus({ id, condition: newCondition }))
                }
            ]
        });
    };
        ...
        <View style={styles.card}>
            <View style={styles.cardHeader}>
                <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.deviceType}>{item.type} • S/N: {item.serialNumber}</Text>
                </View>
            ...
            <TouchableOpacity
                style={[
                    styles.reportButton,
                    { backgroundColor: item.condition === 'working' ? theme.colors.error : theme.colors.success }
                ]}
                onPress={() => item.id && handleReportStatus(item.id, item.condition)}
            >
                <Text style={styles.reportButtonText}>
                    {item.condition === 'working' ? t('devices.reportFaulty') : t('devices.reportWorking')}
                </Text>
            </TouchableOpacity>
        </View>
    );
            ...
            <FlatList
                ...
                ListEmptyComponent={
                    <View style={styles.emptyContainer}>
                        ...
                        <Text style={styles.emptySubText}>{t('devices.noMaterialAssigned')}</Text>
                    </View>
                }
            />
        </View>
