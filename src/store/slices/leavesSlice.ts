...
export const { setLeaves, addLeave, updateLeave, deleteLeave } =
  leavesSlice.actions;

export const selectAllLeaves = (state: RootState) => state.leaves.items;

export const selectUpcomingLeaves = createSelector([selectAllLeaves], items => {
  const now = new Date().toISOString();
  return items
    .filter(
      (l: Leave) =>
        l.status === 'pending' || (l.startDate && l.startDate >= now),
    )
...
export const selectPendingLeaves = createSelector([selectAllLeaves], items =>
  items.filter((l: Leave) => l.status === 'pending'),
);
