...
const devicesSlice = createSlice({
    ...
    reducers: {
        ...
        updateDeviceStatus: (state, action: PayloadAction<{ id: number; condition: 'working' | 'faulty' }>) => {
        ...
        assignDevice: (state, action: PayloadAction<{ id: number; employeeId: number; employeeName: string }>) => {
            const device = state.items.find(item => item.id === action.payload.id);
            if (device) {
...
export const { setDevices, addDevice, updateDevice, deleteDevice, updateDeviceStatus, assignDevice } = devicesSlice.actions;
