...
export const analyticsService = {
  ...
  getAnalytics: async (): Promise<AnalyticsData> => {
    try {
      ...
      const uniqueEmployeesWithLeaves = new Set(
        weeklyLeaves.map(l => l.employeeId),
      ).size;
  ...
  getPayrollAdherence: async (): Promise<{
    labels: string[];
    data: number[];
  }> => {
    const history = await payrollDb.getAllHistory();
    ...
    const adherenceData = last7Days.map(day => {
      const dayHistory = history.filter(
        h => h.paidAt && h.paidAt.startsWith(day.dateStr),
      );
  ...
  getUpcomingLeavesChart: async (): Promise<{
    labels: string[];
    data: number[];
  }> => {
    ...
    leaves.forEach(leave => {
      ...
      const diffDays = Math.floor(
        (leaveDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24),
      );
