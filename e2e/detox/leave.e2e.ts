...
describe('Leave Management', () => {
  ...
  it('should submit leave request successfully', async () => {
    ...
    // Fill in dates
    await element(by.placeholder('YYYY-MM-DD'))
      .atIndex(0)
      .typeText('2023-08-10');
    await element(by.placeholder('YYYY-MM-DD'))
      .atIndex(1)
    ...
    // Fill in reason
    await element(
      by.placeholder('Provide a reason for your leave request'),
    ).typeText('Summer vacation');

    await element(by.text('Submit Request')).tap();

    // Check for success message
    await expect(
      element(by.text('Leave request submitted successfully')),
    ).toBeVisible();
  });
